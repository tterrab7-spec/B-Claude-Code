// music.mjs - composes and renders the DirtBid explainer score to public/audio/music.wav (62 s, stereo, 44.1 kHz).
//
...
const barStart = (bar) => barStarts[bar];
const barLen = (bar) => METER(bar) * BEAT;
const swingFor = (bar) => (bar >= 13 ? 0.11 : bar <= 1 ? 0.06 : 0);
/** Seconds for (bar, beat); beat is 0-based and may be fractional. Off-beat eighths get the section swing. */
function T(bar, beat = 0) {
  let t = barStart(bar) + beat * BEAT;
...
function kick(t, vel = 0.6) {
  const b = BUSES.kick; const tt = Math.max(0, t + rnd.gauss() * b.jitter); const v = velGain(vel * sectionVel);
...
function shaker(t, vel = 0.45, {open = false} = {}) {
  const b = BUSES.shaker; const tt = Math.max(0, t + rnd.gauss() * b.jitter); const v = velGain(vel * sectionVel);
...
const QUARTER = [[0, ['bass', 'top'], 0.6], [0.5, ['mid'], 0.42]];
...
function shakerBar(bar, {vel = 0.45, div = 2, fillFrom = null, fillVel = 0.5} = {}) {
  const beats = METER(bar);
  for (let b = 0; b < beats; b += 1 / div) {
    if (fillFrom != null && b >= fillFrom) break;
    const on = Math.abs(b - Math.round(b)) < 1e-6;
    shaker(T(bar, b), on ? vel : vel * 0.78);
  }
  if (fillFrom != null) for (let b = fillFrom; b < beats; b += 0.25) shaker(T(bar, b), fillVel * (0.8 + 0.2 * (b - fillFrom) / (beats - fillFrom)) * (Math.abs(b - Math.round(b)) < 1e-6 ? 1 : 0.82));
}
...
const stakes = [ // [bar, beat, chord, bassNote, marimbaArp, pizz]
  ...
  [9, 0, 'Am/E', 'E3', ['E5', 'A5', 'C6']], [9, 2, 'Fmaj7hi', 'F3', ['F5', 'A5', 'C6']],
  [10, 0, 'F#dim7', 'F#3', ['F#5', 'A5', 'C6', 'Eb6']], [10, 1, 'E/G#', 'G#3', ['G#5', 'B5', 'E6']],
];
mark(7, 'stakes', 'Am -> G/B: bass A2 B2 half notes, guitar low arpeggios, marimba rising arps, pizz on changes, clock ticks quarters, kick on 1');
mark(8, 'stakes', 'C -> Dm: same, rising (bass C3 D3), ticks + kick on 1 and 3');
mark(9, 'stakes', 'Am/E -> Fmaj7: guitar 8th-note chops, bass quarters E3 F3, shaker enters, ticks 8ths, vibraphone E4+B4');
mark(10, 'stakes', 'F#dim7 -> E/G#: chromatic bass F#3 G#3, then A7 STUTTER (16ths, all hands) beats 3-4, hard DROP-OUT 23.85-24.05, glock pickup A5 C#6');
stakes.forEach(([bar, beat, chord, bn, arp], k) => {
  const prog = k / (stakes.length - 1);
  sectionVel = 0.8 + 0.22 * prog;
  const v = V[chord];
  if (bar <= 8) { // arpeggiated
...
sectionVel = 0.85;
[0, 1, 2].forEach((b) => wb(T(6, b), 0.38, b % 2 === 0));
[0, 1, 2, 3].forEach((b) => wb(T(7, b), 0.4, b % 2 === 0));
[0, 1, 2, 3, 3.5].forEach((b) => wb(T(8, b), 0.44, Math.floor(b) % 2 === 0));
for (let b = 0; b < 4; b += 0.5) wb(T(9, b), 0.46, Math.floor(b) % 2 === 0);
for (let b = 0; b < 2; b += 0.5) wb(T(10, b), 0.52, Math.floor(b) % 2 === 0);
kick(T(7, 0), 0.5); kick(T(8, 0), 0.52); kick(T(8, 2), 0.5);
[0, 1, 2, 3].forEach((b) => kick(T(9, b), 0.52 + 0.02 * b));
[0, 0.5, 1, 1.5].forEach((b) => kick(T(10, b), 0.6 + 0.03 * b));
shakerBar(9, {vel: 0.38});
for (let b = 0; b < 2; b += 0.25) shaker(T(10, b), 0.45);
pad(T(9, 0), ['E4', 'B4'], 0.34, 2.0);
pad(T(10, 0), ['F#4', 'C5'], 0.4, 1.0);
mel('glock', 9, [[0, 'A5', 1, 0.35], [2, 'C6', 1, 0.4]], {ring: true});
mel('glock', 10, [[0, 'Eb6', 1, 0.45]], {ring: true});
// the stutter on A7 (beats 3-4 of bar 10), staccato 16ths, then the drop-out
sectionVel = 1.05;
[2, 2.25, 2.5, 2.75, 3].forEach((b, i) => {
  strum(T(10, b), 'A7low', {vel: 0.72 + 0.03 * i, spread: 0.008, dur: 0.07, release: 0.05});
  bassNote('A2', T(10, b), 0.14, 0.75 + 0.03 * i);
  roll('marimba', T(10, b), ['A4', 'C#5', 'E5'], {vel: 0.7 + 0.03 * i, spread: 0.004, dur: 0.07, release: 0.05});
  note('pizz', 'A2', T(10, b), {dur: 0.08, vel: 0.7, pan: -0.25, release: 0.05});
  kick(T(10, b), 0.68 + 0.03 * i); wb(T(10, b), 0.6, i % 2 === 0); shaker(T(10, b), 0.5);
});
// pickup into the lift (after the silence, gate reopens at 24.05)
sectionVel = 1;
note('glock', 'A5', T(10, 3.5), {vel: 0.42, pan: 0.25, release: 0.4});
note('glock', 'C#6', T(10, 3.75), {vel: 0.5, pan: 0.3, release: 0.4});

// ---------------- TURN (bar 11 4/4 + bar 12 2/4, D major): the lift. Chord hit at 24.35 with glockenspiel sparkle
sectionVel = 1.0;
mark(11, 'turn', 'D MAJOR HIT at 24.35: big guitar strum, bass D2, vibraphone D4 F#4 A4, glock sparkle D6 F#6 A6 D7, kick; kalimba answer F#5 A5 B5 A5');
strum(T(11, 0), 'Dbig', {vel: 0.85, spread: 0.024});
bassNote('D2', T(11, 0), 3.7, 0.8);
pad(T(11, 0), ['D4', 'F#4', 'A4'], 0.5, 3.0);
sparkle(T(11, 0) + 0.005, ['D6', 'F#6', 'A6', 'D7'], 0.62, 0.042);
roll('marimba', T(11, 0), ['D4', 'F#4', 'A4', 'D5'], {vel: 0.55, spread: 0.012, dur: 1.8});
kick(T(11, 0), 0.62); wb(T(11, 0), 0.5); shaker(T(11, 0), 0.42, {open: true});
mel('kalimba', 11, [[2, 'F#5', 0.5, 0.5], [2.5, 'A5', 0.5, 0.5], [3, 'B5', 0.5, 0.55], [3.5, 'A5', 1, 0.45]], {pan: -0.4});
pick(11, 'D', [[2, ['bass', 'top'], 0.45, 1.5], [3, ['alt'], 0.4, 1]]);
mark(12, 'turn', '2/4 fill bar into product: G -> A strums, bass G2 A2 + walk-up B2 C#3, drum fill (kick/woodblock/shaker) from 26.9, marimba pickup A4 B4 C#5');
strum(T(12, 0), 'G', {vel: 0.6, spread: 0.02, dur: 0.9 * BEAT});
bassNote('G2', T(12, 0), 0.9, 0.6);
strum(T(12, 1), 'A', {vel: 0.65, spread: 0.02, dur: 0.6 * BEAT});
bassNote('A2', T(12, 1), 0.45, 0.65); bassNote('B2', T(12, 1.5), 0.22, 0.65); bassNote('C#3', T(12, 1.75), 0.22, 0.7);
note('glock', 'A5', T(12, 1), {vel: 0.35, pan: 0.25, release: 0.4});
kick(T(12, 0), 0.5); kick(T(12, 1), 0.55); kick(T(12, 1.5), 0.62);
[0.5, 1, 1.25, 1.5, 1.75].forEach((b, i) => wb(T(12, b), 0.4 + 0.06 * i, i % 2 === 0));
for (let b = 0.5; b < 2; b += 0.25) shaker(T(12, b), 0.35 + 0.18 * (b - 0.5) / 1.5);
mel('marimba', 12, [[1, 'A4', 0.5, 0.5], [1.5, 'B4', 0.25, 0.55], [1.75, 'C#5', 0.25, 0.6]]);
mel('kalimba', 12, [[0, 'B5', 0.5, 0.42], [0.5, 'G5', 0.5, 0.4]], {pan: -0.4});

// ---------------- PRODUCT (bars 13-18 4/4 + bar 19 2/4, D major): bouncy driving groove, full arrangement
sectionVel = 0.95;
const productChords = {13: 'D', 14: 'A', 15: 'Bm', 16: 'G', 17: 'D', 18: 'A/C#'};
const gPatterns = {13: P1, 14: P2, 15: P1, 16: P3, 17: null, 18: P1};
const bassLines = { // [beat, note, durBeats, vel]
  13: [[0, 'D2', 1.4, 0.62], [1.5, 'D2', 0.45, 0.5], [2, 'A2', 1.0, 0.58], [3.5, 'G#2', 0.45, 0.55]],
  14: [[0, 'A2', 0.9, 0.62], [1, 'A3', 0.45, 0.5], [2, 'E2', 0.9, 0.58], [2.5, 'A2', 0.45, 0.5], [3.5, 'A#2', 0.45, 0.55]],
  15: [[0, 'B2', 1.4, 0.62], [1.5, 'B2', 0.45, 0.5], [2, 'F#2', 1.0, 0.58], [3.5, 'F#2', 0.45, 0.55]],
  16: [[0, 'G2', 1.4, 0.62], [1.5, 'G2', 0.45, 0.5], [2, 'D3', 0.9, 0.56], [3, 'B2', 0.45, 0.5], [3.5, 'C#3', 0.45, 0.55]],
  17: [[0, 'D2', 0.9, 0.64], [1, 'D3', 0.45, 0.5], [2, 'A2', 0.9, 0.58], [2.5, 'D2', 0.45, 0.5], [3.5, 'C#3', 0.45, 0.55]],
  18: [[0, 'C#3', 1.4, 0.62], [1.5, 'C#3', 0.45, 0.5], [2, 'A2', 1.0, 0.58], [3, 'B2', 0.45, 0.5], [3.5, 'A2', 0.45, 0.55]],
  19: [[0, 'G2', 0.9, 0.62], [1, 'A2', 0.45, 0.62], [1.5, 'A2', 0.45, 0.64]],
};
const theme = { // marimba melody, [beat, pitch, durBeats, vel]
  13: [[0, 'D5', 1, 0.62], [1, 'F#5', 0.5, 0.58], [1.5, 'A5', 1, 0.62], [2.5, 'B5', 0.5, 0.58], [3, 'A5', 1, 0.6]],
  14: [[0, 'G5', 0.5, 0.58], [0.5, 'E5', 0.5, 0.55], [1, 'C#5', 1, 0.58], [2, 'E5', 0.5, 0.55], [2.5, 'F#5', 0.5, 0.55], [3, 'E5', 1, 0.58]],
  15: [[0, 'D5', 0.5, 0.6], [0.5, 'F#5', 0.5, 0.58], [1, 'B5', 1.5, 0.64], [2.5, 'A5', 0.5, 0.56], [3, 'F#5', 1, 0.58]],
  16: [[0, 'G5', 1, 0.6], [1, 'B5', 0.5, 0.58], [1.5, 'A5', 0.5, 0.55], [2, 'G5', 1, 0.56]],
  17: [[0, 'D5', 1, 0.64], [1, 'F#5', 0.5, 0.6], [1.5, 'A5', 0.5, 0.6], [2, 'D6', 1, 0.66], [3, 'A5', 1, 0.6]],
  18: [[0, 'C#5', 0.5, 0.6], [0.5, 'E5', 0.5, 0.58], [1, 'A5', 1, 0.62], [2.5, 'G5', 0.5, 0.56], [3, 'E5', 1, 0.58]],
  19: [[0, 'G5', 0.5, 0.6], [0.5, 'B5', 0.5, 0.6], [1, 'D6', 0.25, 0.62], [1.25, 'C#6', 0.25, 0.58], [1.5, 'E6', 0.5, 0.62]],
};
const productText = {
  13: 'D: groove starts - Travis-picked guitar (P1), bouncy bass D2/A2, kick 1+3, shaker 8ths, woodblock 2+4, marimba theme "D F# A B A"',
  14: 'A: guitar P2 (strum on 3), bass A2/E2 with octave, kick pickup on 4&, shaker 16th fill, marimba theme answer, kalimba fill',
  15: 'Bm: guitar P1, extra woodblock on 3&, marimba theme (B5 peak)',
  16: 'G: guitar P3 (syncopated, rests), marimba rests 2nd half, kalimba turn B4 A4 G4, shaker fill',
  17: 'D: strummed bar (P4), bass with octave, pizz on 1, glock doubles marimba; "TA-DA" on beat 3 (37.95, under "within five percent"): glock A5 -> D6+F#6, kalimba D6, open shaker, woodblock',
  18: 'A/C#: guitar P1, bass C#3, marimba + glock',
  19: '2/4 fill bar into partners: G -> A7 quarter patterns, bass G2 A2, drum fill, marimba lick G5 B5 D6 C#6 E6, vibraphone C#4 G4 pad enters (partners crossfade)',
};
for (let bar = 13; bar <= 18; bar++) {
  mark(bar, 'product', productText[bar]);
  const chord = productChords[bar];
  if (gPatterns[bar]) pick(bar, chord, gPatterns[bar]);
  else strumBar(bar, chord, {vel: 0.62});
  for (const [b, p, db, v] of bassLines[bar]) bassNote(p, T(bar, b), db, v);
  mel('marimba', bar, theme[bar]);
  if (bar >= 17) mel('glock', bar, theme[bar].filter((s) => s[2] >= 1).map((s) => [s[0], s[1], s[2], 0.34]), {ring: true});
  // drums
  kick(T(bar, 0), 0.58); kick(T(bar, 2), 0.55);
  if (bar >= 17) kick(T(bar, 1.5), 0.45);
  if (bar === 14 || bar === 16) kick(T(bar, 3.5), 0.5);
  const fill = (bar === 14 || bar === 16) ? 3 : null;
  shakerBar(bar, {vel: 0.45, fillFrom: fill, fillVel: 0.48});
  wb(T(bar, 1), 0.5); wb(T(bar, 3), 0.5);
  if (bar === 15 || bar === 17) wb(T(bar, 2.5), 0.38, false);
}
// bar 19 (2/4): the fill into partners
mark(19, 'product', productText[19]);
pick(19, 'G', QUARTER); pick(19, 'A7', [[1, ['bass', 'hi'], 0.55, 0.5], [1.5, ['top'], 0.45, 0.5]]);
for (const [b, p, db, v] of bassLines[19]) bassNote(p, T(19, b), db, v);
mel('marimba', 19, theme[19]);
mel('glock', 19, [[0, 'G5', 1, 0.34], [1.5, 'E6', 1, 0.36]], {ring: true});
kick(T(19, 0), 0.58); kick(T(19, 1), 0.5); kick(T(19, 1.5), 0.55);
shakerBar(19, {vel: 0.45, fillFrom: 0, fillVel: 0.5});
wb(T(19, 0.5), 0.42, false); [1, 1.25, 1.5, 1.75].forEach((b, i) => wb(T(19, b), 0.5 + 0.07 * i, i % 2 === 0));
// kalimba fills, pizz, pad
mel('kalimba', 14, [[3.5, 'C#5', 0.5, 0.45]], {pan: -0.4});
mel('kalimba', 16, [[2.5, 'B4', 0.5, 0.48], [3, 'A4', 0.5, 0.46], [3.5, 'G4', 0.5, 0.44]], {pan: -0.4});
mel('kalimba', 17, [[3.5, 'F#5', 0.5, 0.42]], {pan: -0.4});
note('pizz', 'D3', T(17, 0), {dur: 0.4, vel: 0.5, pan: -0.25});
pad(T(19, 0), ['C#4', 'G4'], 0.36, 1.1);
// the "ta-da" accent (bar 17 beat 3 = 37.95 s, while the narration says "within five percent"): short, high, out of the voice's way
{
  const TADA = T(17, 2);
  note('glock', 'A5', TADA - 0.25 * BEAT, {vel: 0.4, pan: 0.15, release: 0.45});
  note('glock', 'D6', TADA, {vel: 0.5, pan: 0.25, release: 0.5}); note('glock', 'F#6', TADA + 0.012, {vel: 0.42, pan: 0.4, release: 0.5});
  note('kalimba', 'D6', TADA, {vel: 0.5, pan: -0.3});
  shaker(TADA, 0.42, {open: true}); wb(TADA, 0.5);
}

// ---------------- PARTNERS (bars 20-23, D major): fuller and warmer, pizzicato + vibraphone pads
sectionVel = 0.95;
const partnersText = {
  20: 'Bm7: vibraphone pad F#4 A4 D5, pizz offbeats, guitar P2, bass B2/F#2, marimba warm variation, glock sparkle on the change',
  21: 'Gmaj7: vibraphone G4 B4 D5, pizz offbeats, guitar P1, bass G2/D3, kalimba pickup',
  22: 'D/F# -> Em7: guitar half patterns, bass F#2 E2 walk, vibraphone pads, pizz countermelody G3 B3, woodblock 3&',
  23: 'A7sus -> A7: strums, driving bass A2, pads E4 G4 A4 -> C#4 E4 G4, kalimba rising fill A4 B4 C#5 E5, shaker fill into CTA',
};
for (let bar = 20; bar <= 23; bar++) mark(bar, 'partners', partnersText[bar]);
pick(20, 'Bm7', P2); pick(21, 'Gmaj7', P1); pick(22, 'D/F#', HALF); pick(22, 'Em7', shift(HALF, 2));
strum(T(23, 0), 'A7sus', {vel: 0.55, dur: 1.4 * BEAT}); strum(T(23, 1.5), 'A7sus', {vel: 0.5, dur: 0.45 * BEAT});
strum(T(23, 2), 'A7', {vel: 0.6, dur: 0.9 * BEAT}); strum(T(23, 3), 'A7', {vel: 0.5, up: true, dur: 0.3}); strum(T(23, 3.5), 'A7', {vel: 0.55, dur: 0.25});
const pBass = {
  20: [[0, 'B2', 1.4, 0.62], [1.5, 'B2', 0.45, 0.5], [2, 'F#2', 1.0, 0.58], [3.5, 'A2', 0.45, 0.55]],
  21: [[0, 'G2', 1.4, 0.62], [1.5, 'G2', 0.45, 0.5], [2, 'D3', 0.9, 0.56], [3, 'D2', 0.45, 0.5], [3.5, 'E2', 0.45, 0.55]],
  22: [[0, 'F#2', 1.4, 0.62], [1.5, 'A2', 0.45, 0.5], [2, 'E2', 0.9, 0.6], [3, 'G2', 0.45, 0.5], [3.5, 'G#2', 0.45, 0.55]],
  23: [[0, 'A2', 1.4, 0.64], [1.5, 'A2', 0.45, 0.52], [2, 'E2', 0.9, 0.6], [3, 'A2', 0.45, 0.58], [3.5, 'A2', 0.45, 0.62]],
};
for (const bar of [20, 21, 22, 23]) for (const [b, p, db, v] of pBass[bar]) bassNote(p, T(bar, b), db, v);
pad(T(20, 0), ['F#4', 'A4', 'D5'], 0.4); pad(T(21, 0), ['G4', 'B4', 'D5'], 0.4);
pad(T(22, 0), ['F#4', 'A4', 'D5'], 0.38, 1.0); pad(T(22, 2), ['E4', 'G4', 'B4'], 0.38, 1.0);
pad(T(23, 0), ['E4', 'G4', 'A4'], 0.4, 1.0); pad(T(23, 2), ['C#4', 'E4', 'G4'], 0.42, 1.0);
const pizzOff = {20: ['F#3', 'D4'], 21: ['B3', 'D4'], 22: ['A3', 'G3'], 23: ['E4', 'C#4']};
for (const bar of [20, 21, 22, 23]) { note('pizz', pizzOff[bar][0], T(bar, 1.5), {dur: 0.4, vel: 0.5, pan: -0.25}); note('pizz', pizzOff[bar][1], T(bar, 3.5), {dur: 0.4, vel: 0.5, pan: -0.25}); }
mel('pizz', 22, [[2.5, 'G3', 0.5, 0.48], [3, 'B3', 0.5, 0.5]], {pan: -0.25, release: 0.1});
mel('pizz', 23, [[0.5, 'A3', 0.5, 0.48], [1, 'G3', 0.5, 0.46]], {pan: -0.25, release: 0.1});
const themeB = {
  20: [[0, 'F#5', 1, 0.6], [1, 'D5', 0.5, 0.55], [1.5, 'E5', 0.5, 0.55], [2, 'F#5', 1.5, 0.6]],
  21: [[0, 'G5', 0.5, 0.58], [0.5, 'A5', 0.5, 0.56], [1, 'B5', 1, 0.62], [2.5, 'D5', 0.5, 0.52], [3, 'E5', 0.5, 0.54], [3.5, 'F#5', 0.5, 0.56]],
  22: [[0, 'A5', 1, 0.62], [1, 'F#5', 0.5, 0.56], [1.5, 'D5', 0.5, 0.54], [2, 'E5', 1, 0.58], [3, 'G5', 0.5, 0.56], [3.5, 'F#5', 0.5, 0.55]],
  23: [[0, 'E5', 1.5, 0.6], [2, 'C#5', 0.5, 0.56], [2.5, 'D5', 0.5, 0.56], [3, 'E5', 0.5, 0.58], [3.5, 'G5', 0.5, 0.6]],
};
for (const bar of [20, 21, 22, 23]) { mel('marimba', bar, themeB[bar]); mel('glock', bar, [[0, themeB[bar][0][1].replace(/\d/, (d) => +d + 1), 1, 0.3]], {ring: true}); }
sparkle(T(20, 0), ['B5', 'D6', 'F#6'], 0.4, 0.05);
mel('kalimba', 21, [[3.5, 'D5', 0.5, 0.42]], {pan: -0.4});
mel('kalimba', 23, [[2.5, 'A4', 0.5, 0.46], [3, 'B4', 0.5, 0.48], [3.25, 'C#5', 0.25, 0.5], [3.5, 'E5', 0.5, 0.54]], {pan: -0.4});
for (const bar of [20, 21, 22, 23]) {
  kick(T(bar, 0), 0.58); kick(T(bar, 2), 0.55);
  if (bar === 21 || bar === 23) kick(T(bar, 3.5), 0.5);
  shakerBar(bar, {vel: 0.45, fillFrom: bar === 23 ? 2 : null, fillVel: 0.52});
  wb(T(bar, 1), 0.5); wb(T(bar, 3), 0.5);
  if (bar === 22) wb(T(bar, 2.5), 0.38, false);
}
[3, 3.25, 3.5, 3.75].forEach((b, i) => wb(T(23, b), 0.52 + 0.07 * i, i % 2 === 0));

// ---------------- CTA (bars 24-26, D major): biggest, cheerful; final cadence lands at 58.35 (bar 27)
sectionVel = 1.05;
mark(24, 'cta', 'D: tutti - strummed guitar (P4), bass D2/A2, kick 1 2& 3 4&, shaker 16ths, marimba theme + glock doubling, pizz offbeats, vibraphone D4 F#4 A4');
mark(25, 'cta', 'Bm7 -> G: strums, bass B2 / G2, pads B3 D4 F#4 -> G3 B3 D4, marimba theme answer + glock');
mark(26, 'cta', 'A7sus -> A7: driving 8th-note bass A2, strums, drum fill (kick/woodblock/shaker), marimba + glock rising run A..A6');
mark(27, 'cta', 'FINAL D MAJOR at 58.35: big strum D3-D5, bass D2, marimba + vibraphone D4 F#4 A4 D5, glock sparkle D6 F#6 A6 D7 (+ A6 D7 twinkle), kick/woodblock hit; rings out, silence by 61.4');
strumBar(24, 'D', {vel: 0.7});
strum(T(25, 0), 'Bm7', {vel: 0.68, dur: 0.9 * BEAT}); strum(T(25, 1), 'Bm7', {vel: 0.5, up: true, dur: 0.3}); strum(T(25, 1.5), 'Bm7', {vel: 0.6, dur: 0.45 * BEAT});
strum(T(25, 2), 'G', {vel: 0.7, dur: 0.9 * BEAT}); strum(T(25, 3), 'G', {vel: 0.5, up: true, dur: 0.3}); strum(T(25, 3.5), 'G', {vel: 0.6, dur: 0.45 * BEAT});
strum(T(26, 0), 'A7sus', {vel: 0.7, dur: 0.9 * BEAT}); strum(T(26, 1), 'A7sus', {vel: 0.55, up: true, dur: 0.3}); strum(T(26, 1.5), 'A7sus', {vel: 0.62, dur: 0.45 * BEAT});
strum(T(26, 2), 'A7', {vel: 0.72, dur: 0.45 * BEAT}); strum(T(26, 2.5), 'A7', {vel: 0.62, up: true, dur: 0.22}); strum(T(26, 3), 'A7', {vel: 0.75, dur: 0.45 * BEAT}); strum(T(26, 3.5), 'A7', {vel: 0.62, dur: 0.18});
const ctaBass = {
  24: [[0, 'D2', 1.4, 0.66], [1.5, 'D2', 0.45, 0.54], [2, 'A2', 1.0, 0.6], [3.5, 'B2', 0.45, 0.58]],
  25: [[0, 'B2', 1.4, 0.66], [1.5, 'B2', 0.45, 0.54], [2, 'G2', 1.4, 0.64], [3.5, 'G2', 0.45, 0.56]],
  26: [[0, 'A2', 1.4, 0.68], [1.5, 'A2', 0.45, 0.56], [2, 'A2', 0.45, 0.64], [2.5, 'A2', 0.45, 0.6], [3, 'A2', 0.45, 0.68], [3.5, 'A2', 0.45, 0.66]],
};
for (const bar of [24, 25, 26]) for (const [b, p, db, v] of ctaBass[bar]) bassNote(p, T(bar, b), db, v);
mel('marimba', 24, theme[13]); mel('glock', 24, theme[13].filter((s) => s[2] >= 1).map((s) => [s[0], s[1], s[2], 0.38]), {ring: true});
mel('marimba', 25, [[0, 'B5', 1, 0.62], [1, 'A5', 0.5, 0.58], [1.5, 'F#5', 1, 0.6], [2.5, 'G5', 0.5, 0.58], [3, 'B5', 0.5, 0.6], [3.5, 'A5', 0.5, 0.6]]);
mel('glock', 25, [[0, 'B5', 1, 0.36], [1.5, 'F#5', 1, 0.34], [3, 'B5', 0.5, 0.36]], {ring: true});
const run = ['A4', 'B4', 'C#5', 'D5', 'E5', 'F#5', 'G5', 'A5'];
mel('marimba', 26, run.map((p, i) => [i < 4 ? i * 0.5 : 2 + (i - 4) * 0.5, p, 0.5, 0.6 + 0.03 * i]));
mel('glock', 26, ['A5', 'B5', 'C#6', 'D6', 'E6', 'F#6', 'G6', 'A6'].map((p, i) => [2 + i * 0.25, p, 0.25, 0.44 + 0.03 * i]), {ring: true});
pad(T(24, 0), ['D4', 'F#4', 'A4'], 0.42); pad(T(25, 0), ['B3', 'D4', 'F#4'], 0.4, 1.0); pad(T(25, 2), ['G3', 'B3', 'D4'], 0.4, 1.0);
pad(T(26, 0), ['A3', 'D4', 'E4'], 0.42, 1.0); pad(T(26, 2), ['A3', 'C#4', 'G4'], 0.45, 1.0);
for (const bar of [24, 25]) { const tones = bar === 24 ? ['A3', 'F#4'] : ['D4', 'B3']; note('pizz', tones[0], T(bar, 1.5), {dur: 0.4, vel: 0.54, pan: -0.25}); note('pizz', tones[1], T(bar, 3.5), {dur: 0.4, vel: 0.54, pan: -0.25}); }
for (let b = 0; b < 4; b += 0.5) note('pizz', 'A3', T(26, b), {dur: 0.3, vel: 0.5 + 0.04 * b, pan: -0.25});
// drums
kick(T(24, 0), 0.62); kick(T(24, 1.5), 0.5); kick(T(24, 2), 0.6); kick(T(24, 3.5), 0.52);
kick(T(25, 0), 0.62); kick(T(25, 2), 0.6); kick(T(25, 3.5), 0.52);
[0, 1, 2, 2.5, 3, 3.5].forEach((b, i) => kick(T(26, b), 0.6 + 0.03 * i));
shakerBar(24, {vel: 0.48, div: 4});
shakerBar(25, {vel: 0.46});
for (let b = 0; b < 4; b += 0.25) shaker(T(26, b), (0.42 + 0.1 * b / 4) * (Math.abs(b - Math.round(b)) < 1e-6 ? 1 : 0.82));
for (const bar of [24, 25]) { wb(T(bar, 1), 0.54); wb(T(bar, 3), 0.54); }
wb(T(24, 2.5), 0.4, false);
wb(T(26, 1), 0.54); [2.5, 2.75, 3, 3.25, 3.5, 3.75].forEach((b, i) => wb(T(26, b), 0.5 + 0.06 * i, i % 2 === 0));
// the final chord
sectionVel = 1.1;
const F0 = T(27, 0);
...
if (process.env.DEBUG) {
  console.log('bus levels (post-gain RMS dBFS): whole | product 28-42 s | peak');
  for (const [n, b] of Object.entries(BUSES)) { const g = D.dB(b.gainDb); console.log(`  ${n.padEnd(10)} ${(D.toDb(D.rms(b.buf) * g)).toFixed(1).padStart(6)} ${(win(b.buf, 28, 42) + b.gainDb).toFixed(1).padStart(6)} ${(D.toDb(D.peak(b.buf) * g)).toFixed(1).padStart(6)}`); }
}
...
// master chain: high-pass 40 Hz, gentle bus compression, fade to silence by 61.4
...
D.applyCurve(master, (t) => (t < 60.3 ? 1 : t < 61.4 ? Math.pow(1 - (t - 60.3) / 1.1, 1.6) : 0));
...
if (process.env.DEBUG) {
  ...
  const hot = []; for (let bar = 0; bar <= 27; bar++) { const a = D.resize({sr: D.SR, ch: pre.ch.map((c) => c.subarray(D.secs(T(bar)), D.secs(T(bar) + barLen(bar))))}, barLen(bar)); const p = D.toDb(D.truePeak(a)); if (p > CEILING_DB) hot.push(`bar ${bar}: ${p.toFixed(1)}`); }
...
// level checks against the picture: per-scene RMS arc, the gate, the hit and the final chord
console.log('\nscene RMS (dBFS, final mix): ' + timeline.scenes.map((s) => `${s.id} ${win(final, s.start, Math.min(s.end, TOTAL)).toFixed(1)}`).join(' | '));
console.log(`gate: before ${win(final, GATE_AT - 0.25, GATE_AT).toFixed(1)} dB, closed ${(GATE_AT + 0.01).toFixed(2)}-${GATE_OPEN} s ${win(final, GATE_AT + 0.01, GATE_OPEN).toFixed(1)} dB, pickup ${GATE_OPEN}-${HIT} s ${win(final, GATE_OPEN, HIT).toFixed(1)} dB; ` +
  `hit ${HIT}-${(HIT + 0.2).toFixed(2)} s ${win(final, HIT, HIT + 0.2).toFixed(1)} dB (peak ${D.toDb(D.peak(D.resize({sr: D.SR, ch: final.ch.map((c) => c.subarray(D.secs(HIT), D.secs(HIT + 0.2)))}, 0.2))).toFixed(1)} dBFS); ` +
...
for (const a of arrangement) {
  const t0b = T(a.bar), t1b = t0b + barLen(a.bar);
  ...
  console.log(`${String(a.bar).padStart(3)}  ${METER(a.bar)}/4   ${t0b.toFixed(2).padStart(5)}-${t1b.toFixed(2).padStart(5)}  ${a.section.padEnd(9)} ${counts}\n     ${a.text}`);
}
